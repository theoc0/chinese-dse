(() => {
  ...
  const STORAGE_PREFIX = "fanwen_game_progress_v1";

  const stageRanges = [
    { start: 1, end: 5, plan: { 1: 20, 2: 10 } },
    { start: 6, end: 10, plan: { 1: 12, 2: 12, 3: 6 } },
    { start: 11, end: 15, plan: { 2: 14, 3: 10, 4: 6 } },
    { start: 16, end: 20, plan: { 3: 12, 4: 12, 5: 6 } },
    { start: 21, end: 25, plan: { 4: 12, 5: 12, 6: 6 } },
    { start: 26, end: 30, plan: { 5: 12, 6: 12, 7: 6 } },
    { start: 31, end: 35, plan: { 6: 12, 7: 12, 8: 6 } },
    { start: 36, end: 40, plan: { 7: 12, 8: 12, 9: 6 } },
    { start: 41, end: 45, plan: { 8: 12, 9: 12, 10: 6 } },
    { start: 46, end: 50, plan: { 6: 6, 7: 6, 8: 6, 9: 6, 10: 6 } }
  ];

  const bank = window.BANK_FANWEN || null;
  ...
  function normalizeQuestion(q, idx) {
    ...
    return {
      id: q.id || `fanwen-${idx + 1}`,
      source: q.source || "範文全文",
      type: q.type || "內容理解",
  ...
  function renderQuiz() {
    el.quizForm.innerHTML = "";
    el.quizTitle.textContent = `範文全文闖關版｜第 ${state.currentStage} 關`;
    el.quizSubtitle.textContent = `本關共 ${QUESTIONS_PER_STAGE} 題，選項順序每次隨機。`;
  ...
  function showBankError() {
    ...
    el.resultSummary.innerHTML = `
      bank-fanwen.js 載入失敗或格式不正確。<br>
      請確認：<br>
      1. bank-fanwen.js 已成功上傳<br>
      2. 檔案內有 window.BANK_FANWEN<br>
      3. questions 陣列存在<br>
      4. index.html 的 script 路徑正確<br>
      5. 瀏覽器已清除快取或使用版本號
    `;
  }

  function startCurrentStage() {
    if (!window.BANK_FANWEN || !Array.isArray(window.BANK_FANWEN.questions)) {
      showBankError();
      return;
    }
    ...
    if (!state.stageQuestionSet.length) {
      el.resultSummary.classList.remove("empty-state");
      el.resultSummary.textContent = "目前沒有可用題目。";
      return;
    }
